...
|---|---|---|---|---|---|
| 0-SLOPE | **SLOPE CHECK the v9.4 WDL net (157-wdlnet) on Lichess val, then rescale the output head if it is off >5%** -- measured 6 Sep iter 40 over 261 paired (static, Stockfish-reference) samples in the post-mortem JSONs: our static/reference slope was 0.82 in rounds 4-19 and 0.27 in rounds 20-32 (nets shrunk toward zero) but **1.34 in rounds 37-41, the v9.4 era** -- the net now overstates by ~a third. RFP / razoring / futility / the NMP margin all compare that static against fixed cp thresholds, so a shouting eval clears them too easily and prunes the refutation (the `152-sfnet` failure verbatim). Pre-registered direction: DOWN, ~x0.75. Caveats: `static` is logged only on flagged moves (selected sample), v9.4 cell is n=58 | unknown, but it mis-scales every pruning margin | ~0 (one forward pass) | Lichess-val slope first; a rescale is a net task like `155-mixnet2s`, judged on the SPRT only | NET LANE, before the next net ships |
