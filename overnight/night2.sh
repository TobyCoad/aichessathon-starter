...
if ! finished review; then
    say "=== review by Claude Fable ==="
    CLAUDE="$HOME/.local/bin/claude.exe"
    if [ -x "$CLAUDE" ]; then
        sed "s#@STATE@#$STATE#g" overnight/REVIEW_PROMPT.md | "$CLAUDE" --print --model fable --permission-mode dontAsk > "$STATE/REVIEW.md" 2> "$STATE/review.err"
        say "review exit $?, $(wc -c < "$STATE/REVIEW.md") bytes"
    fi
    mark review
fi
say "=== night 2 done ==="
