...
# The human is waiting on the v9.4 email. If any stage of the net fails, ship the search
# bundle rather than block the release: one gauntlet either way, just without the net.
fallback() { say "$1 -- queueing a SEARCH-ONLY v9.4"; $PY training/queue_v94.py "$SEDV94" --no-net; exit 1; }

DEADLINE=$(( $(date +%s) + 4200 ))   # 70 min: the decode is ~15 min when the machine is free
while ! grep -q "^done:" overnight/eval/sf-decode-wdl.log 2>/dev/null; do
    [ "$(date +%s)" -gt "$DEADLINE" ] && fallback "decode did not finish in 70 min"
    sleep 30
done
...
if [ ! -f training/checkpoints/net_w512-b8-kz16-wdl.json ]; then
    $PY -u training/train.py \
        ...
        > overnight/eval/train-wdl.log 2>&1 || fallback "TRAIN FAILED"
fi
...
if cmp -s overnight/nets/157-wdlnet.npz weights/net.npz; then
    fallback "the WDL net is identical to the tree net"
fi
