...
CUDA torch (cu128, for the RTX 5070's sm_120) is installed. Confirm
`torch.cuda.is_available()` before starting; if it is False, **stop and record it in
the journal** rather than training on CPU for a night.
