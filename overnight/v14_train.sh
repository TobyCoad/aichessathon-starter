...
if [ ! -f training/checkpoints/net_v14.json ]; then
    $PY -u training/train.py --data $SHARDS --val data/mixed_val.npy \
        ...
        --lr 3e-5 --epochs 100 --patience 40 --warmup-epochs 0 --skip-sanity --limit 40000000 \
        --out training/checkpoints/net_v14.pt > overnight/eval/train-v14.log 2>&1 || { say "v13 TRAIN FAILED"; exit 1; }
fi
say "v13 train done: $(grep -E 'restored|wrote.*json' overnight/eval/train-v14.log | tail -n 2 | tr '\n' ' ')"
...
if false; then
    $PY -u training/train.py --data $EG --val data/mixed_val.npy \
        --resume training/checkpoints/net_v14.pt --mirror \
        --accumulator 512 --buckets 8 --king-zones 16 \
...
evaluate 210-v13 training/checkpoints/net_v14.pt
:
...
a=$(grep -oE "now [0-9,]+" overnight/eval/retest-220-v14.log | tr -d ',' | awk '{print $2}')
b=$(grep -oE "now [0-9,]+" overnight/eval/retest-220-v14-s400.log | tr -d ',' | awk '{print $2}')
pick=220-v14; [ "${b:-99999}" -lt "${a:-99999}" ] && pick=220-v14-s400
...
$PY -u -m testing.gauntlet --challenger "overnight/challengers/$pick" --champion overnight/challengers/200-mixmirror-s400 \
    --games 400 --workers 4 > overnight/eval/v14-vs-v12.gauntlet.log 2>&1
say "gauntlet result: $(grep -E '^(PROMOTE|REJECT|INCONCLUSIVE)|score' overnight/eval/v14-vs-v12.gauntlet.log | tail -n 2 | tr '\n' ' ')"
say "done"
